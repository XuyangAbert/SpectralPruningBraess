...
def proxydelmin(data, nxgraph,seed, max_iterations):
    ...
    newgraph = min_and_update_edges(nxgraph, rank_by_proxy_delete_min, "proxydeletemin", max_iter=max_iterations)
...
def proxyaddmax(data, nxgraph, seed,max_iterations):
    ...
    #             diff_class_diff_community_before += 1
    start_algo = time.time()
    newgraph = process_and_update_edges(nxgraph, rank_by_proxy_add, "proxyaddmax", max_iter=max_iterations)
    newgraph.remove_edges_from(list(nx.selfloop_edges(newgraph)))
    end_algo = time.time()
...
def proxyaddmin(data, nxgraph,seed, max_iterations):
    ...
    labels = data.y.cpu().numpy()
    start_algo = time.time()
    newgraph = min_and_update_edges(nxgraph, rank_by_proxy_add_min, "proxyaddmin", max_iter=max_iterations)
    newgraph.remove_edges_from(list(nx.selfloop_edges(newgraph)))
    end_algo = time.time()
    # Track the edges after addition
    updated_edges = set(newgraph.edges())
